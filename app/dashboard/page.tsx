...
// ── React component ───────────────────────────────────────────
// Realtime channels live on the module-level Supabase client and survive
// remounts (client-side nav, dev Strict Mode). Subscribing twice throws, so
// only the first mount wires them up; data loads are idempotent and re-run.
let _rtSubscribed = false
export default function Page() {
  useEffect(() => {
    loadData(); loadCCData(); loadARData(); loadRNData()
    if (!_rtSubscribed) {
      _rtSubscribed = true
      subscribeRealtime(); subscribeCCRealtime(); subscribeARRealtime(); subscribeRNRealtime()
    }
    setTimeout(()=>{
      const pill=el('statusPill'),txt=el('statusText')
  ...
  return (
    <>
      ...
      {/* Fleet manager voice + text query widget — dispatches FleetQueryAgent via
          ...
          Rendered as a plain <script> tag (not next/script) because the widget bundle must
          load as a classic script tag — next/script's afterInteractive/lazyOnload strategies
          inject it via document.createElement after hydration, which LiveKit's loader does
          not support. React 19 hoists a real <script> element here into a genuine part of
          the rendered HTML. */}
      <script
